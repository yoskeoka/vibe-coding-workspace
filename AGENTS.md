...
2.  **Branch & PR Rules**:
    ...
    - When the PR is ready for human review, include the PR URL and current worktree path so a human can run or patch the branch locally without searching for it. End the handoff with a compact separate-session prompt that future follow-up for the same PR should continue in a new session. Keep the prompt itself to about 3 lines and include the PR number, branch name, requested follow-up scope, PR URL, and worktree path.
