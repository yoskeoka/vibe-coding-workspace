...
- Planning PRs may stop after that initial poll when no other stop-condition work remains. Execution PRs must keep required CI completion in the landing check: if required checks are still pending after the initial poll, add up to two more `30-second wait -> poll` turns before handoff, unless checks finish earlier, advisory-review waiting starts, the head SHA changes, the helper fails, or the human asks to stop.
