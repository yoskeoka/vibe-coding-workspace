...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Frame extraction interval in seconds. Default is conservative for CPU-only machines.",
    )
    parser.add_argument(
        "--segment-length-sec",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            "Small scheduler batch size for OCR work. Keep this low on WSL2/CPU-only setups; "
            f"{MAX_RECOMMENDED_OCR_BATCH_SIZE} is the recommended ceiling."
        ),
    )
    parser.add_argument(
        "--ocr-lang",
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.ocr_batch_size > MAX_RECOMMENDED_OCR_BATCH_SIZE:
        raise SystemExit(
            f"--ocr-batch-size {args.ocr_batch_size} is too high for the intended WSL2/CPU-first setup. "
